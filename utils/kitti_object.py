...
def show_image_with_dt_dt_boxes(img, dt_objects, gt_objects, save_path=None, hull=None):
    ...
    h, w, c = img.shape
    plt.imshow(img1)  # the x in sitk is z in ITKSnap
    # img3 = np.copy(img)  # for 3d bbox
    # TODO: change the color of boxes
    # print(dt_objects, gt_objects)
    for objects, color in zip([gt_objects, dt_objects], ['r', 'b']):
        ...
        for obj in objects:
            ...
            #         plt.text(obj.xmin, obj.ymin, str(obj.conf), fontsize=5, color=color)  # TODO: str format.0
            #     plt.gca().add_patch(
            #         plt.Rectangle((obj.xmin, obj.ymin), obj.xmax - obj.xmin,
            #                       obj.ymax - obj.ymin, fill=False,
            #                       edgecolor=color,
            #                       linewidth=1))  # attention, plt.Rectangle's x is the y for image and y is image's x
            #
            #
            # box_colormap = [
            #     [1, 1, 1],
    ...
    if hull is not None:
        ...
        # hull = np.insert(hull, 0, hull[0])
        print(hull.shape)
        plt.plot(hull[:, 0], hull[:, 1], 'r--', lw=1)
        # plt.plot(imgfov_pts_2d[hull, 0], imgfov_pts_2d[hull, 1], 'r--^', lw=2)
        # for i in range(len(hull) - 1):
        #     plt.text(hull[i, 0], hull[i, 1], str(i), fontsize=20)
...
def my_show_lidar_on_image(pc_velo, img, calib, img_width, img_height):
    """ Project LiDAR points to image """
    img = np.copy(img)
    imgfov_pc_velo, pts_2d, fov_inds = get_lidar_in_image_fov(
        pc_velo, calib, 0, 0, img_width, img_height, True
    ...
    for i in tqdm(range(imgfov_pts_2d.shape[0])):
        ...
        if y_index >= img_width:
            y_index = img_width - 1
        if x_index >= img_height:
            x_index = img_height - 1
        img[x_index,
            y_index] = (255, 255, 0)

        # plt.scatter(int(np.round(imgfov_pts_2d[i, 0])),
