...
class MyObject2d(object):
    """ 2d object label """

    def __init__(self, arr, tp, conf):
        self.xmin = arr[0]  # left
        self.ymin = arr[1]  # top
        self.xmax = arr[2]  # right
        self.ymax = arr[3]  # bottom
        self.type = tp
        self.conf = conf
...
def extrinsic_mutaion(V2C, theta_xyz_velo, trans_xyz_cam):
    V2C_noise = np.zeros_like(V2C)
    theta_xyz_velo = [theta * math.pi / 180.0 for theta in theta_xyz_velo]
    theta_xyz_velo = np.array(theta_xyz_velo)

    # trans_xyz_cam = [1 + trans for trans in trans_xyz_cam]
    # trans_xyz_cam = np.array(trans_xyz_cam)

    # add rotaion noise
    angles = rotationMatrixToEulerAngles(V2C[:, :3])
    angles += theta_xyz_velo  # add noise
    V2C_rotation_noise = eulerAnglesToRotationMatrix(angles)
    V2C_noise[:, :3] = V2C_rotation_noise

    # add tans noise
    # V2C_trans_noise = V2C[:, 3] * trans_xyz_cam
    V2C_trans_noise = V2C[:, 3] + trans_xyz_cam
    # print(V2C[:, 3])
...
class Calibration(object):
    ...
    def __init__(self, calib_filepath, from_video=False, camera="P2"):
        ...
        self.P = calibs[camera]  # mark nei can
        self.P = np.reshape(self.P, [3, 4])
        # Rigid transform from Velodyne coord to reference camera coord
        self.V2C = calibs["Tr_velo_to_cam"]  # mark wai can
        self.V2C = np.reshape(self.V2C, [3, 4])
        # mark noise
        # self.V2C = extrinsic_mutaion(self.V2C, [0, 0, -10], [0, 0, 0])
        # mark noise
    ...
    # y(Camera) = P * R0_rect*Tr_velo_to_cam *x (Lidar)
...
def load_gray_image(img_filename):
