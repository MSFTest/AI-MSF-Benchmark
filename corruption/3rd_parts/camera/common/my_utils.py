...
def fill_depth(depth, img_height, img_width):
    height = depth.shape[0]
    width = depth.shape[1]

    fill_x1 = int((img_height - height) / 2)
    fill_x2 = img_height - height - fill_x1

    fill_y1 = int((img_width - width) / 2)
    fill_y2 = img_width - width - fill_y1

    depth = cv2.copyMakeBorder(depth, fill_x1, fill_x2, fill_y1, fill_y2, borderType=cv2.BORDER_REPLICATE)
    return depth
