...
class DBManager:
    ...
    def load_streaks_from_xml(self, dataset, settings, image_shape_WH, use_pickle=True, verbose=True):
        ...
        try:
            ...
            raise Exception(
                "Reading XML file {} crashed, which is likely due to corrupted particles simulation files. If so, delete this simulation folder manually and re-run to allow generation of new simulation.".format(
        ...
        try:
            for fix, frame in enumerate(simulation):
                ...
                for drop in frame:
                    ...
                    s.image_position_start = np.array(drop.attrib["ip1"][1:-1].split(';'), dtype=float) / settings[
                        "render_scale"]  # x,y
                    s.image_position_end = np.array(drop.attrib["ip2"][1:-1].split(';'), dtype=float) / settings[
                        "render_scale"]  # x,y
...
class RainRenderer:
    ...
    def add_drop_to_image(self, dataset, env_map_xyY, solid_angle_map, drop_fov_pts, drop_minC, bg, rainy_bg,
        ...
        if rendering_strategy in ['white']:
            ...
            if 'mask_env_float64' not in cache or not np.all(
                    solid_angle_map.shape[:2] == cache['mask_env_bool'].shape[:2]):
...
class EnvironmentMapGenerator:
    ...
    def generate_map(self, background):
        ...
        # print("*******************")
        # print(mask.shape, mask_temp.shape, cyl.shape, cyl_temp.shape, cv2.flip(cyl, 0).shape,
        #       self.fillMatDown.shape)
        # print(np.where(mask_temp == 0)[0])
        # print(cyl_temp[np.where(mask_temp == 0)[0], np.where(mask_temp == 0)[1]].shape)
        # print("*******************", self.fillMatDown[:, 0].shape)
        # print("*******************", self.fillMatDown[:, 1].shape)
        # print("*******************", cyl.shape)
        # print("___________________", mask_temp.shape)
        # print("___________________", cyl_temp.shape)
        # xxxx = cv2.flip(cyl, 0)[self.fillMatDown[:, 0], self.fillMatDown[:, 1]]

        # yyy = cyl_temp[np.where(mask_temp == 0)[0], np.where(mask_temp == 0)[1]]
        # print(yyy.shape, xxxx.shape)
