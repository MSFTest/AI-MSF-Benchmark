...
class Generator:
    def __init__(self, args):
        ...
        # output paths
        self.output_root = os.path.join(args.output)
    ...
    def check_folders(self):
        ...
        for sequence in self.sequences:
            for w in self.weather:
                ...
                out_dir = os.path.join(self.output_root, '{}mm'.format(w["fallrate"]))
    ...
    def run(self):
        ...
        for folder_idx, sequence in enumerate(self.sequences):
            ...
            for sim_idx, sim_weather in enumerate(self.weather):
                ...
                out_seq_dir = os.path.join(self.output_root)
                out_dir = os.path.join(out_seq_dir)
                ...
                if "nuscenes" in self.dataset:
                    ...
                    files = natsorted(np.array([os.path.join(self.images[sequence], picture) for picture in
                                                my_utils.os_listdir(self.images[sequence])]))
                    depth_files = natsorted(
                        np.array([os.path.join(depth_folder, depth) for depth in my_utils.os_listdir(depth_folder)]))
                    im = files[0]
                    if im.endswith(".png"):
                ...
                self.renderer = RainRenderer(focal=self.focal, f_number=self.f_number, focus_plane=6, radius=10,
                                             fov=165)
                ...
                self.db.load_streaks_from_xml(self.dataset, self.settings, [imW, imH], use_pickle=False,
                ...
                if self.frames:
                    ...
                    idx = list(range(f_start, f_end - 1, f_step))  # to make it
                ...
                for f_idx, i in enumerate(idx):
                    ...
                    ori_shape = bg.shape
                    _flag = False
                    # TODO: cut bug assert for even num not odd num
                    if bg.shape[0] % 2 != 0:
                        bg = bg[:bg.shape[0] - 1, :]
                        _flag = True

                    if _flag:
                        print(ori_shape, "to", bg.shape)
                    if self.settings["render_scale"] != 1:
                        bg = cv2.resize(bg, (int(bg.shape[1] // self.settings["render_scale"]),
                                             int(bg.shape[0] // self.settings["render_scale"])))
                    ...
                    if FOG_ATT == 1:
                        ...
                        if depth_file.endswith(".png"):
                            ...
                            if depth.shape[0] % 2 != 0:
                                depth = depth[:depth.shape[0] - 1, :]
                            if depth.shape[1] % 2 != 0:
                                depth = depth[:, :depth.shape[1] - 1]
                            if depth is None:
                                print('Missing/Corrupted depth data (%s)' % depth_file)
                                continue
                            if not np.all(depth.shape[:2] == bg.shape[:2]):
                                # TODO:  fill depth match image
                                print("xxxxxxxxxxxxxxxxx", depth.shape, bg.shape)
                                depth = my_utils.fill_depth(depth, bg.shape[0], bg.shape[1])
                                plt.imsave("./depth.png", depth)
                                assert depth.shape[0] == bg.shape[0]
                                assert depth.shape[1] == bg.shape[1]
                        ...
                        depthHW = np.array(
                            [int((depth.shape[0] * self.settings["depth_scale"]) // self.settings["render_scale"]),
                             int((depth.shape[1] * self.settings["depth_scale"]) // self.settings["render_scale"])])
                        ...
                        if not np.all(depth.shape[:2] == bg.shape[:2]):
                            print(
                                "\nDepth {} size ({},{}) differs from image ({},{}). Will assume depth is crop centered.".format(
                                    image_file, depth.shape[0], depth.shape[1], bg.shape[0], bg.shape[1]))
                            # ori settings
