...
def check_arg(args):
    ...
    parser.add_argument('--interest_directory',
                        help='current directory insterest',
                        required=False)
    ...
    assert os.path.exists(results.texture), (
        "rainstreakdb database is not valid. Some files are missing.", results.texture)
    assert os.path.exists(results.norm_coeff), (
        "rainstreakdb database is not valid. Some files are missing.", results.norm_coeff)
    ...
    # Filter sequences
    results.sequences = np.asarray(
        [seq for seq in results.sequences if np.any([seq[:len(_s)] == _s for _s in sequences_filter])])
    ...
    for seq in results.sequences:
        ...
        if not os.path.exists(results.depth[seq]):
            ...
            valid = False
        if results.calib[seq] is not None and not np.all(
                [os.path.exists(f) for f in results.calib[seq]] if isinstance(results.calib[seq],
                                                                              list) else os.path.exists(
                    results.calib[seq])):
    ...
    for seq in results.sequences:
        ...
        weathers_to_run = [w for w in results.weather if len(glob2.glob(
            my_utils.particles_path(results.particles[seq]["path"], w))) == 0 or results.force_particles]
        if len(weathers_to_run) != 0:
            sims_to_run.append(
                {"path": [results.particles[seq]["path"]], "options": [results.particles[seq]["options"]],
                 "weather": weathers_to_run})

    if len(sims_to_run) == 0:
        print(" All particles simulations ready")
    else:
        print(
            " {} particles simulations to compute... Simulations will attempt to run automatically using weather particle simulator".format(
                len(sims_to_run)))
        for sim in sims_to_run:
            import tools.particles_simulation
            tools.particles_simulation.process(sim,
                                               force_recompute=True)  # Current script already decided it has to be re-run
    ...
    for seq in results.sequences:
        try:
            particles2[seq] = [glob2.glob(my_utils.particles_path(results.particles[seq]["path"], w))[0] for w in
                               results.weather]
        except Exception:
