...
class LISA():
    def __init__(self, m=1.328, lam=905, rmax=200, rmin=1.5, bdiv=3e-3, dst=0.05,
        ...
        if saved_model:
            ...
            # print("Extinction efficiency", self.qext, self.qback)
        else:
            try:
                dat = np.load('mie_q.npz')
                self.D = dat['D']
        ...
        elif atm_model == 'low_advection_fog':
            self.N_model = lambda D: self.Nd_low_advection_fog(D)
    ...
    def lisa_mc(self, x, y, z, ref, Rr):
        ...
        if ran > rmin:
            ...
            Nt = np.int32(
                np.floor(Nt) + (np.random.rand() < Nt - int(Nt)))  # convert to integer w/ probabilistic rounding
        else:
            Nt = 0
        ...
        if Nt > 0:
            ...
            Pr = ref_r * np.exp(-2 * alpha * ran_r) * np.minimum((Dr / Db(ran_r)) ** 2, np.ones(Dr.shape)) / (
                    ran_r ** 2)
            if (self.mode == 'strongest'):
                ...
                if P0 < Pmin and Pr[ind_r] < Pmin:  # if all smaller than Pmin, do nothing
                    ...
                    ref_new = ref_r * np.exp(-2 * alpha * ran_new) * np.minimum((Dr[ind_r] / Db(ran_r[ind_r])) ** 2,
                                                                                1)  # new reflectance biased by scattering
    ...
    def augment_avg(self, pc):
        ...
        alpha, beta = self.alpha_beta(Nd)  # extinction coeff. (1/m)
        # print("extinction coeff", alpha, beta)
        ...
        ran_new[indp] = ran[indp] + np.random.normal(0,
                                                     sig[indp])  # range with uncertainty added, keep range 0 if P<Pmin
        ref_new = ref * np.exp(-2 * alpha * ran)  # new reflectance modified by scattering
    ...
    def haze_point_cloud(self, pts_3D, Rr=0):
        ...
        elif (self.atm_model == 'chu_hogg_fog') or (self.atm_model == 'strong_advection_fog') or (
                self.atm_model == 'moderate_advection_fog'):
        ...
        cloud_scatter[:, 0:3] = np.transpose(np.multiply(np.transpose(cloud_scatter[:, 0:3]), np.transpose(
            np.divide(dnew[cloud_scatter_idx], d[cloud_scatter_idx]))))
        ...
        subsampled_idx = np.random.choice(len(random_scatter_idx), int(fraction_random * len(random_scatter_idx)),
                                          replace=False)
        ...
        random_scatter[:, 0:3] = np.transpose(
            np.multiply(np.transpose(random_scatter[:, 0:3]), np.transpose(drand / d[random_scatter_idx])))
    ...
    def calc_Mie_params(self):
        ...
        qext = out[1]
        qback = out[6]
        # print("Extinction efficiency", qext)
    ...
    # Moderate advection fog
    def Nd_low_advection_fog(self, D):
