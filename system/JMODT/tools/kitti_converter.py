    ...
    for seq in range(21):
        ...
        with open(tracking_label, 'r') as f:
            ...
            for line in lines:
                ...
                obj += str(tid) + "\n"
                # if "DontCare" in obj:
                #     dont_care_obj.append(obj)
                # else:
                label_dict[the_frame].append(obj)
            # for obj in dont_care_obj:
            #     label_dict[the_frame].append(obj)

        for frame in frames:
            sample_str = str(sample_id).zfill(6)
            if not only_labels:
                # print("***", os.path.join(tracking_image, f'{frame}.png'))
            ...
            with open(os.path.join(res_label, f'{sample_str}.txt'), 'w+') as f:
                ...
                for s in temp_d:
                    temp.append(s)
                # print(frame,label_dict[frame],os.path.join(res_label, f'{sample_str}.txt'))
                f.writelines(temp)
