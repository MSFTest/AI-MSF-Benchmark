...
fieldnames = [
    'epoch', 'rmse', 'photo', 'mae', 'irmse', 'imae', 'mse', 'tmae', 'trmse', 'absrel', 'lg10',
...
class logger:
    def __init__(self, args, prepare=True, output_directory=None):
        self.args = args
    ...
    def conditional_print(self, split, i, epoch, lr, n_set, blk_avg_meter,
        ...
        if (i + 1) % self.args.print_freq == 0:
            ...
            #     'TMAE = {blk_avg.tmae:.3f}({average.tmae:.3f})\n\t '
            #     'TRMSE = {blk_avg.trmse:.3f}({average.trmse:.3f}) '
            #         .format(epoch,
            #                 i + 1,
            #                 n_set,
            #                 lr=lr,
    ...
    def conditional_save_info(self, split, average_meter, epoch):
        ...
        if split == "train":
            ...
            return avg
        elif split in ["test", 'val_selection', 'full']:
            eval_filename = os.path.join(self.output_directory, 'eval.txt')
            self.save_single_txt(eval_filename, avg, epoch)
            return avg
        else:
    ...
    def conditional_summarize(self, mode, avg, is_best, curr_step=0):
        print("\n*\nSummary of ", mode, "round")
        print(''
              'RMSE={average.rmse:.3f}\n'
              'MAE={average.mae:.3f}\n'
              'TMAE={average.tmae: .3f}\n'
              'TRMSE={average.trmse: .3f}\n'
              ...
              't_GPU={time:.3f}\n'
              'curr_step = {curr_step: }'.format(average=avg, time=avg.gpu_time, curr_step=curr_step))
        if is_best and mode == "val":
    ...
    def loss_summarize(self, mode, trainloss, valloss, duration, curr_step=0):
        print('curr_step {:d},trainloss_tot = {:.7f}, valloss_tot = {:.7f}, ({:.7f} sec/step)'.format(curr_step,
                                                                                                      trainloss,
                                                                                                      valloss,
                                                                                                      duration))

    def loss_summarize_v2(self, mode, trainloss, trainlossmaince, valloss, vallossmaince, duration, curr_step=0):
        print(
            'curr_step {:d},trainloss_tot = {:.7f}, train_maince = {:.7f}, valloss_tot = {:.7f}, valmaince = {:.7f}, ({:.7f} sec/step)'.format(
                curr_step,
                trainloss, trainlossmaince, valloss, vallossmaince, duration))

    def loss_summarize_v3(self, mode, trainloss, trainlossmaince, trainlossfusion, valloss, vallossmaince,
                          vallossfusion, duration, curr_step=0):
        print(
            'curr_step {:d},trainloss_tot = {:.7f}, train_maince = {:.7f}, train_fusion = {:7f}, valloss_tot = {:.7f}, valmaince = {:.7f}, valloss_fusion = {:7f}, ({:.7f} sec/step)'.format(
                curr_step,
                trainloss, trainlossmaince, trainlossfusion, valloss, vallossmaince, vallossfusion, duration))
...
def save_checkpoint_modified(state, epoch, output_directory, is_best=True, curr_step=None):
    ...
    checkpoint_filename = os.path.join(output_directory,
                                       'checkpoint-' + str(epoch) + '_' + str(curr_step) + '.pth.tar')
    torch.save(state, checkpoint_filename)
