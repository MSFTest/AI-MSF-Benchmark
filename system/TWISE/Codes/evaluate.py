...
def iterate(mode, args, loader1, model, optimizer, logger, epoch, curr_step=None):
    ...
    fns = natsorted(os.listdir("../Data/depth_selection/val_selection_cropped/velodyne_raw"))
    for i, batch_data in tqdm(enumerate(loader1)):
        ...
        if mode in ['test_completion']:
            ...
            with torch.no_grad():
                ...
                if args.save_imageflag == 'T':
                    ...
                    # print(pred_dep.shape)
                    im = Image.fromarray(pred_dep)
                    im = im.convert('L')
                    im.save(os.path.join(save_path, fns[i]))
                ...
                #     i_arr.append(i)
                # if avg.rmse > 2000:
                #     logger.conditional_summarize(mode, avg, is_best, i)  # mark
                curr_step += 1
